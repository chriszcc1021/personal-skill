...
for sd in skill_dirs:
    ...
    total = (scores["structure"]+scores["reuse"]+scores["exec"]+ref_s+scores["fresh"]) * 4   # 5×5 → 100 制
    skills_data.append({
...
# Bayesian-weighted average to avoid small-sample bias.
# Scale: each skill total is 5-25 (sum of 5 dims, each 1-5).
# Prior: pretend each author also has prior_n virtual skills at prior_avg.
# Small samples get pulled toward prior_avg; large samples barely move.
BAYES_PRIOR_N = 5
BAYES_PRIOR_AVG = 71  # 100制中等偏上基准
skill_quality_avg = {}
buckets = collections.defaultdict(list)
for s in skills_data:
    buckets[s["author"]].append(s["total"])
for a, lst in buckets.items():
    adj = (sum(lst) + BAYES_PRIOR_N * BAYES_PRIOR_AVG) / (len(lst) + BAYES_PRIOR_N)
    skill_quality_avg[a] = adj

# ---- Commit messages (load before daily activity so subjects are available) ----
commit_msg = {}
...
daily = collections.defaultdict(lambda: collections.defaultdict(lambda: {"commits":0,"churn":0,"tops":[]}))
for sha, c in commits.items():
    name = canon(c["email"])
    dt = c["date"][:10]
    ...
    subj = (commit_msg.get(sha, {}).get("subject") or "")[:50]
    if subj and not subj.lower().startswith(("merge ","merge:")):
        daily[name][dt]["tops"].append({"sha": sha[:7], "subject": subj, "churn": churn})
        # keep top 3 by churn
        daily[name][dt]["tops"].sort(key=lambda x: -x["churn"])
        daily[name][dt]["tops"] = daily[name][dt]["tops"][:3]
...
for sha, c in commits.items():
    ...
    churn = sum(ins+dels for _,ins,dels in c["files"])
    files_n = len(c["files"])
    # Classify commit by file paths touched
    paths = [p for p,_,_ in c["files"]]
    has_skill = any("/skills/" in p or p.startswith("skills/") or "SKILL.md" in p for p in paths)
    has_tool  = any("/tool-stations/" in p or p.startswith("tool-stations/") for p in paths)
    if has_skill:   kind = "SKILL"
    elif has_tool:  kind = "TOOL"
    else:           kind = "KNOWLEDGE"
    if subj.lower().startswith(("merge ","merge:")): continue
    if churn < 5: continue
    top_commits[name].append({
        "sha": sha[:7], "date": c["date"][:10], "subject": subj[:90],
        "churn": churn, "files": files_n, "kind": kind,
    })
